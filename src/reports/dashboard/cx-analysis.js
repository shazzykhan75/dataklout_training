...
const CXAnalysis = (props) => {
    ...
    function fetchData() {
        services.get(`api/dashboard/cx_score_analysis/`).then(res => {
            console.log(res);
            // setIsPending(false);
            if (res == 'TypeError: Failed to fetch') {
                // setError('Connection Error');
    ...
    const [cxDataSeries, setCXDataSeries] = useState(null);
    ...
    function createDummyData() {
        try {
                ...
                if (fData.products[j].status) {
                    ...
                    for (let i = 1; i <= dateDiff; i++) {
                        ...
                        if (deactivate_agent == total_agent) {
                            ...
                            score = (tempData[j][i] * 0.9)
                        }
                        else {
                            score = (tempData[9 - j][50 - i - deactivate_index]) * 0.8
                        }
            ...
            // setCXDetail([total * 0.5, total * 0.1, total * 0.35, total * 0.05])
            ...
            console.log(cxSeries)
            setCXDataSeries(cxSeries);
                ...
                <div style={{}}>
                    <div className="assistant" style={{ height: "450px" }}>
                        <div style={{ paddingTop: "20px", paddingLeft: "40px" }}>
                            <label style={{ fontSize: "20px", color: "#271078" }}><BsGraphUp /> &nbsp;CX Score Trend</label>
                        </div>
